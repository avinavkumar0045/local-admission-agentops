...
# We fix the Data Link syntax. Grafana's TraceQL needs the ID to be quoted 
# if we are doing a direct lookup, or we switch the queryType to "traceql" with proper braces.
dashboard_payload = {
  "dashboard": {
    "id": None,
    ...
    "panels": [
      ...
      {
        "type": "table",
        "title": "Recent Queries & Clickable Traces",
        "gridPos": { "h": 8, "w": 12, "x": 0, "y": 14 },
        "datasource": { "uid": "bfv8qs0pp6osga", "type": "mysql" },
        "targets": [
          { "format": "table", "rawSql": "SELECT trace_id, start_time, status, duration_ms FROM spans WHERE span_type = 'full_query_lifecycle' ORDER BY start_time DESC LIMIT 15", "refId": "A" }
        ],
        "fieldConfig": {
          "overrides": [
            {
              ...
              "properties": [
                {
                  ...
                  "value": [
                    {
                      "title": "Open 11-Step Waterfall",
                      "targetBlank": True,
                      "url": "",
                      "internal": {
                        "datasourceUid": "ffx7prdoh20hsd",
